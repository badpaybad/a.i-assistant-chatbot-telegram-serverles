...
#### Cách 1: Chạy qua Local Web Server
...
Nhấp chuột để chọn file `results.csv` thủ công hoặc kéo thả file vào giao diện để xem.

#### Cách 2: Mở tệp trực tiếp và Kéo & Thả (Drag & Drop)
Bạn có thể mở trực tiếp tệp [index.html](file:///work/a.i-assistant-chatbot-telegram-serverles/cameraip/train/train_dashboard/index.html) trong trình duyệt qua giao thức `file://` (bằng cách nhấp đúp vào file):
1. Kéo tệp `results.csv` từ thư mục chạy của bạn (ví dụ: `cameraip/train/runs/detect/train_tiny/results.csv`) thả vào vùng chỉ định trên màn hình.
2. Hoặc nhấp nút **"Chọn kết quả (CSV)"** để tìm và chọn file kết quả thủ công từ máy tính của bạn.
