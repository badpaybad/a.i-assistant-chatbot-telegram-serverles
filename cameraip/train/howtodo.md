...
> 1. **Hỗ trợ cấu trúc CVAT tự động**: Tự động duyệt tìm file ảnh và nhãn `.txt` đệ quy sâu bên trong các thư mục con như `obj_train_data/` mà không bắt buộc bạn phải di chuyển file hoặc đổi tên thư mục.
> 2. **Tự động trích xuất Tên Class**: Đọc trực tiếp từ file `obj.names` (được sinh ra khi xuất dữ liệu từ CVAT) để đưa tên các nhãn (ví dụ: `test`) vào file `dataset.yaml` thay vì tạo các nhãn mặc định `class_0`, `class_1` giúp bạn không cần chỉnh sửa thủ công cấu hình `dataset.yaml`.
