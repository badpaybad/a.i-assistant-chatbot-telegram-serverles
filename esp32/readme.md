...
2. ESP32 chỉ cần cấu hình kết nối mạng LAN và Firebase Firestore. Khi bắt đầu hội thoại (sau wake-word), ESP32 tự động kết nối qua WebSocket local tới hub để thực hiện stream âm thanh 2 chiều (gửi PCM 16kHz Mono nhị phân và nhận PCM 24kHz Mono nhị phân).
3. Tuy nhiên, Web UI cấu hình của ESP32 vẫn duy trì ô nhập Gemini API Key cho mục đích tương thích hoặc nâng cấp cấu hình sau này.
