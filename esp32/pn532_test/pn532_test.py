...
while True:
    print("\n" + "="*60)
    print("CHUẨN BỊ QUÉT THẺ MỚI")
    print("="*60)
    
    # Nhập thông tin BAC cho thẻ chuẩn bị quét
    so_cccd_input = input(f"Nhập Số CCCD (Enter để dùng {pn532_config.SO_CCCD}): ").strip()
    if not so_cccd_input:
        SO_CCCD = pn532_config.SO_CCCD
        NGAY_SINH = pn532_config.NGAY_SINH
        NGAY_EXP = pn532_config.NGAY_EXP
    else:
        SO_CCCD = so_cccd_input
        NGAY_SINH = input("Nhập Ngày sinh (Định dạng YYMMDD): ").strip()
        NGAY_EXP = input("Nhập Ngày hết hạn (Định dạng YYMMDD): ").strip()
        
    print(f"Số CCCD: {SO_CCCD}")
    print(f"Ngày sinh: {NGAY_SINH}")
    print(f"Ngày hết hạn: {NGAY_EXP}")
    # Khởi tạo Epoch Time cho phiên này
    ...
    try:
        ...
        if resp_auth and resp_auth[-2:] == b'\x90\x00':
            ...
            for idx, text in enumerate(unique_texts):
                print(f" [{idx+1}] {text}")
                text_info_buffer+=f" [{idx+1}] {text}\n"
            
            # TỔNG HỢP VÀ LƯU FILE TEXT
            with open(f"{epoch_time}_info.txt", "w", encoding="utf-8") as f:
                f.write(text_info_buffer)
            print(f"[+] Đã xuất file thông tin: {epoch_time}_info.txt")
            ...
            with open(str(epoch_time)+ "_cccd_security.sod", "wb") as f:
        ...
        # Gửi tín hiệu Soft Reset xuống ESP32 để kết thúc giao dịch thẻ hiện tại
        ser.write(b'\xFF\xFF')
        print("[*] Đã nhả kết nối thẻ. Rút thẻ ra để tiếp tục...")
        time.sleep(2)
        ser.reset_input_buffer()
