...
Hệ thống Backend C# hiện tại được thiết kế theo mô hình **Modular Monolith** kết hợp với **Clean Architecture**, **CQRS**, **Hybrid Authentication/Session**, **Server-Side Paging chuẩn hóa** và **Realtime UI Notification qua Firebase Firestore**.

Khi chuyển đổi (migrate) sang ngôn ngữ **Python**, mục tiêu tối thượng là: **Bảo tồn 100% tư duy kiến trúc, các khái niệm cốt lõi (concepts), tính cô lập nghiệp vụ (Isolation), và đảm bảo khả năng bảo trì, mở rộng đường dài mà không tạo nợ kỹ thuật (Tech Debt).**
...
| **Cấu trúc Solution** | Modular Monolith (.csproj riêng biệt) | **Python Package Namespaces** (`core/infra/...`, `modules/...`) | Phân chia package độc lập, cấm import chéo giữa các `modules`. |
...
| **Data Access Layer** | Entity Framework Core (EF Core) | **SQLAlchemy 2.0 (AsyncEngine + AsyncSession)** | Hỗ trợ Async native, Declarative Mapping, cô lập DB Engine theo từng module. |
...
### 2.1. Cấu Trúc Thư Mục Dự Án Python (Modular Monolith)
...
| **Architectural Patterns** | **Architect / Senior** | Clean Architecture, CQRS, DDD Concepts, Loose Coupling, Event-Driven Architecture, Modular Monolith. |
...
#### Câu 4: Trong một kiến trúc Modular Monolith bằng Python, làm sao để đảm bảo 2 module nghiệp vụ (ví dụ `FilesFolders` và `NhanDienKhuonMat`) không bị lặp mã nguồn (DRY) nhưng vẫn giữ tính cô lập tuyệt đối (Isolation), không import chéo code của nhau?
* **Mục đích kiểm tra**: Tư duy Loose Coupling và tuân thủ quy chuẩn `tot-dev`.
* **Kỳ vọng câu trả lời**:
  1. Không bao giờ `import modules.filesfolders...` trực tiếp trong `modules.nhan_dien_khuon_mat`.
  2. Mọi giao tiếp giữa 2 module phải thông qua **In-Memory CQRS Event Bus / Redis Pub-Sub** hoặc các **Contracts/Interfaces** định nghĩa tại `core.infra.base`.
  3. Nếu module B cần đọc dữ liệu từ DB của module A, module B tạo một `DbContext` read-only phụ của riêng nó mapping đến bảng của module A (như đã quy định ở tài liệu backend howtodo.md C#).
...
### Phần 5: Bài Tập Migration Thực Tế C# -> Python (Live Coding Challenge)
