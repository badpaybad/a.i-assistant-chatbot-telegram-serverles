...
export class CqrsTestComponent implements OnDestroy {
  ...
  sendTestCommand(): void {
    ...
    this.dashboardService.sendTestCommand(this.commandPayload).subscribe({
      next: (res) => {
        ...
        // Listen to Firestore for command result
        const unsub = this.firebase.subscribeToRequestId(res.trackingId, (data) => {
          this.lastResult = data;
          this.notification.success('Command Result (Firestore)', `Result for ${res.trackingId}: ${JSON.stringify(data)}`, { nzDuration: 0 });
        });
