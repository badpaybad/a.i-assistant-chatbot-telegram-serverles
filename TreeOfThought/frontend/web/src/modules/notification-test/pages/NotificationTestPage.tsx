...
const NotificationTestPage: React.FC = () => {
  ...
  const onTriggerNotification = async (values: any) => {
    ...
    try {
      ...
      const unsubscribe = subscribeToRequest(requestId, (data) => {
        ...
        notification.success({
          ...
          description: (
            <div>
              <Paragraph>
                Backend has sent data to path:{" "}
                <Tag color="blue">{requestId}</Tag>
              </Paragraph>
              <pre
                style={{
                  background: "#f5f5f5",
      ...
      await httpClient.post(`/FirebaseTest/notify?path=requests/${requestId}`, {
        ...values,
      ...
      message.loading({
        content: "Waiting for backend to process...",
        key: "noti",
      });
      setTimeout(
        () =>
  ...
  return (
    <div style={{ maxWidth: 800, margin: "0 auto", padding: "24px" }}>
      <Title level={2}>
        <BellOutlined /> Firestore Notification Test
      </Title>
      <Paragraph>
        This module tests the real-time notification flow:
        <strong> Frontend Subscribe</strong> → <strong>Backend Publish</strong>{" "}
        → <strong>Frontend Receive</strong>.
      </Paragraph>
      ...
      <Card
        ...
        extra={
          <Button onClick={() => setRequestId(crypto.randomUUID())}>
            Regenerate ID
          </Button>
        }
      >
        <Form layout="vertical" onFinish={onTriggerNotification}>
          <Form.Item label="Target Request ID (Firestore Path)">
            ...
            <Text type="secondary" size="small">
              This ID is used as the Firestore document path for subscription.
            </Text>
          </Form.Item>
          ...
          <Form.Item
            ...
            initialValue="Hello from Frontend!"
          >
            <Input.TextArea
              rows={2}
              placeholder="Enter message to be sent back via Firestore"
            />
          </Form.Item>
      ...
      <div style={{ marginTop: 40 }}>
        ...
        <ul>
          <li>
            <Tag color="green">Done</Tag> Subscribe to Request ID (Firestore
            Document)
          </li>
          <li>
            <Tag color="green">Done</Tag> Call BE to process/notify
          </li>
          <li>
            <Tag color="green">Done</Tag> Receive data via{" "}
            <code>onSnapshot</code>
          </li>
          <li>
            <Tag color="green">Done</Tag> Show manual-close notification (
