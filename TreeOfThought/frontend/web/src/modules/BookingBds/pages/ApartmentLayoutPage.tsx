        ...
        Modal.confirm({
            ...
            onOk: async () => {
                try {
                    ...
                    const unsubscribe = CqrsOnCommandResult(requestId, (data: any) => {
            ...
            <Row gutter={[16, 16]}>
                {apartments.map(apt => (
                    <Col key={apt.id} xs={12} sm={8} md={6} lg={4}>
                        ...
                        >
                            <Typography.Paragraph>Tầng: {apt.floor}</Typography.Paragraph>
                            <Typography.Paragraph strong>{apt.price.toLocaleString()} VNĐ</Typography.Paragraph>
                            <Button 
                                type="primary" 
