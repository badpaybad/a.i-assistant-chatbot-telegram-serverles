...
public interface IInsightFaceSkiaService : IDisposable
{
    ...
    float CompareVector(float[] v1, float[] v2);
    (float Similarity, FaceInfo? f1, FaceInfo? f2, float[]? v1, float[]? v2) CompareFaces(SKBitmap img1, SKBitmap img2, float detThresh = 0.5f);
}
