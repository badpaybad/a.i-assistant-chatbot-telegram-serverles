...
2.  **Hồ sơ tài liệu (Docs)**: Tạo thư mục `TreeOfThought/docs/quan-ly-luong-nhan-vien/` chứa file `whattodo.md` và `howtodo.md`.
...
```mermaid
graph TD
    Step1[Bước 1: Đọc kỹ whattodo.md & Phân tích CSDL/Code hiện tại] --> Step2[Bước 2: Viết kế hoạch giải pháp kỹ thuật vào howtodo.md]
...
1.  **Bước 1: Phân Tích & Nghiên Cứu**:
    *   Đọc kỹ file `whattodo.md` của nghiệp vụ đó để hiểu rõ yêu cầu và ý đồ của người dùng. Nếu chưa có file `whattodo.md`, yêu cầu người dùng đưa srs/mong muốn của họ vào file này trước.
    *   Khảo sát mã nguồn hiện tại trong repository để hiểu rõ base infra và tránh viết lại các chức năng đã có.
2.  **Bước 2: Thiết Kế Giải Pháp (howtodo.md)**:
    *   Viết toàn bộ thiết kế kiến trúc, cấu trúc CSDL dự kiến, sơ đồ luồng dữ liệu, APIs, các thay đổi chi tiết và kế hoạch kiểm thử vào file `howtodo.md` của nghiệp vụ đó.
...
3.  **Bước 3: Lấy Phê Duyệt Của Người Dùng**:
    *   Thông báo cho người dùng xem trước và đánh giá file `howtodo.md`.
...
4.  **Bước 4: Triển Khai Thực Thi & Xác Thực**:
    *   Sau khi nhận được lệnh "tiến hành" của người dùng, bắt đầu viết code thực thi chuẩn chỉnh theo đúng giải pháp đã cam kết tại `howtodo.md`.
