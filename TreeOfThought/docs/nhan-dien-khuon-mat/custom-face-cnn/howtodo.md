...
## 📌 Cập nhật 2: Cơ chế Tự Thích Ứng Nghiêng, Che Khuất & Camera Test Hỗ Trợ Góc Chéo

Để đối phó hiệu quả với các trường hợp người dùng quay nghiêng mặt, chéo góc, hoặc bị che khuất một phần (như đeo khẩu trang, kính mắt), hệ thống sử dụng kết hợp bộ trích xuất landmarks 3D và thuật toán Dynamic Attention Fusion trong kiểm thử thực tế.

### 1. Quy trình xử lý trên Camera Test
* **Định vị & Align mặt thời gian thực**: Sử dụng Haar Cascades (hoặc MediaPipe) để phát hiện khuôn mặt từ luồng camera. Sau đó, MediaPipe Face Mesh trích xuất 26 điểm landmarks đặc trưng chính.
* **Cắt phân vùng động**: Dựa trên landmarks thu được, luồng camera tự động thực hiện phép xoay Affine để đưa khuôn mặt về tư thế thẳng, sau đó cắt thành 3 luồng ảnh độc lập (`x_global` $112 \times 112$, `x_eye` $112 \times 56$, `x_nose` $56 \times 56$) và trích xuất vector tọa độ `x_geom`.
* **Trực quan hóa trọng số tự chú ý**: Trên giao diện camera (`camera_test.py`), hệ thống hiển thị HUD (Heads-Up Display) hiển thị trực tiếp các trọng số Attention thu được từ cổng `attention_weights` (`w_eye`, `w_nose`, `w_global`).
  - Khi đeo khẩu trang, trọng số của vùng mũi sẽ tự động tiệm cận `0.0`, trong khi vùng mắt tăng vọt lên để gánh phần lớn độ tin cậy nhận diện.
  - Khi quay nghiêng, trọng số vùng toàn cảnh (`w_global`) sẽ tăng lên để bao quát các đặc trưng khung xương mặt còn lại.

### 2. Tự động dọn dẹp cơ sở dữ liệu khi chạy thử nghiệm
Để đảm bảo camera test luôn sử dụng cơ sở dữ liệu tương thích với mô hình vừa huấn luyện, script `camera_test.py` được thiết kế để tự động xóa file lưu trữ vector cũ (`faiss_faces.index` và `user_ids.txt`) trước khi chạy, sau đó tự động đăng ký lại các mẫu khuôn mặt từ thư mục `dataraw` bằng mô hình ONNX mới.

---

## 📌 Cập nhật 3: Giám Sát Tiến Trình Huấn Luyện & Đánh Giá Tự Động Với Flask & Gemini API

Để tối ưu hóa trải nghiệm nhà phát triển và theo dõi trực quan quá trình hội tụ của mô hình, hệ thống được trang bị một hệ thống giám sát và đánh giá tự động kép:

### 1. Ghi nhận nhật ký huấn luyện chuẩn hóa
Trong quá trình huấn luyện, log huấn luyện được in ra console theo định dạng chuẩn hóa kèm dấu mốc thời gian chi tiết:
```text
[2026-06-03 16:15:30] [BATCH_PROGRESS] Epoch 1/10 | Batch 50/120 | Loss: 2.1405 | Acc: 42.50%
```
Tất cả các thông số huấn luyện (Loss, Accuracy, Validation Loss, Validation Accuracy) được lưu trữ đồng thời vào tệp tin `train_log.csv` theo từng epoch.

### 2. Dashboard Web Giám Sát (Flask & Chart.js)
Tệp [app.py](file:///work/a.i-assistant-chatbot-telegram-serverles/TreeOfThought/docs/nhan-dien-khuon-mat/custom-face-cnn/app.py) triển khai một ứng dụng Flask UI trực quan:
* **Tính năng**: Cho phép bắt đầu (Start), tạm dừng (Stop/Kill) tiến trình huấn luyện ngay trên giao diện web.
* **Đồ thị thời gian thực**: Sử dụng Chart.js để vẽ biểu đồ đường xu hướng hội tụ của Loss và Accuracy (cho cả tập Train và Validation) bằng cách đọc dữ liệu động từ `train_log.csv`.
* **Hiển thị nhật ký**: Live log stream hiển thị các dòng log từ tiến trình chạy ngầm.

### 3. Đánh giá chất lượng tự động bằng Gemini API
Cứ sau mỗi Epoch huấn luyện, script [evaluator.py](file:///work/a.i-assistant-chatbot-telegram-serverles/TreeOfThought/docs/nhan-dien-khuon-mat/custom-face-cnn/evaluator.py) sẽ tự động chạy:
* **Nguyên lý**: Đọc lịch sử 10 epoch gần nhất từ `train_log.csv`. Sử dụng khóa API `GEMINI_APIKEY` và mô hình `GEMINI_MODEL` được định nghĩa trong `config_dunp.py` để gửi yêu cầu đánh giá đến Gemini API.
* **Đầu ra**: Gemini sẽ phân tích xu hướng hội tụ, đưa ra cảnh báo nếu mô hình bị quá khớp (overfitting) hoặc có tốc độ học quá chậm, và lưu trữ nhận định này vào tệp `train_evaluations.txt` cũng như ghi đè vào `train.log`.

---

## 📌 Cập nhật 4: Tích Hợp Mô hình ONNX Trên Đa Nền Tảng (C#, JS/TS, Dart/Flutter) & Trích Xuất 468 Landmarks

Sau khi huấn luyện thành công và xuất ra tệp `custom_face_cnn.onnx`, mô hình có thể được tích hợp trực tiếp trên các nền tảng client/server khác nhau mà không cần cài đặt thư viện PyTorch cồng kềnh.

### 1. Trích xuất toàn bộ 468 Landmarks với MediaPipe Face Mesh
Để có đầu vào `x_geom` cho mô hình, client cần chạy MediaPipe Face Mesh (tải mô hình dạng local hoặc CDN) để lấy toàn bộ 468 điểm tọa độ 3D chuẩn hóa.
Sau đó, lọc ra đúng danh sách 26 landmarks cốt lõi theo thứ tự index quy định:
* Mắt trái: `[33, 133, 159, 145, 463, 341, 256, 252, 283, 353, 357, 362]`
* Mũi & nhân trung: `[1, 4, 197, 6, 168, 8, 10, 338, 297, 332, 284, 251, 389, 356]`
* Vành mặt & cằm: `[152, 377, 400, 378, 379, 365, 397, 288, 361, 323, 454, 356, 389, 251, 284, 332]`
...
public class FaceEmbeddingExtractor {
    ...
    public float[] Extract(float[] globalImg, float[] eyeImg, float[] noseImg, float[,] geomCoords) {
        // Khởi tạo các Tensor tương ứng với shape quy định
        var tGlobal = new DenseTensor<float>(globalImg, new[] { 1, 3, 112, 112 });
        var tEye = new DenseTensor<float>(eyeImg, new[] { 1, 3, 56, 112 });
        var tNose = new DenseTensor<float>(noseImg, new[] { 1, 3, 56, 56 });
        var tGeom = new DenseTensor<float>(geomCoords.Cast<float>().ToArray(), new[] { 1, 26, 2 });

        var inputs = new List<NamedOnnxValue> {
            NamedOnnxValue.CreateFromTensor("x_global", tGlobal),
            NamedOnnxValue.CreateFromTensor("x_eye", tEye),
            NamedOnnxValue.CreateFromTensor("x_nose", tNose),
            NamedOnnxValue.CreateFromTensor("x_geom", tGeom)
        };

        using var results = _session.Run(inputs);
        var embedding = results.First(r => r.Name == "face_embedding").AsTensor<float>().ToArray();
        return L2Normalize(embedding);
    }

    private float[] L2Normalize(float[] vector) {
        float sum = vector.Sum(x => x * x);
...
#### B. JavaScript / TypeScript (Node.js & Browser via ONNX Runtime Web)
```typescript
import * as ort from 'onnxruntime-web';

async function getFaceEmbedding(
    globalTensor: ort.Tensor,
    eyeTensor: ort.Tensor,
    noseTensor: ort.Tensor,
    geomTensor: ort.Tensor
): Promise<Float32Array> {
    const session = await ort.InferenceSession.create('./custom_face_cnn.onnx');
    
    const feeds = {
        x_global: globalTensor, // Shape: [1, 3, 112, 112]
        x_eye: eyeTensor,       // Shape: [1, 3, 56, 112]
        x_nose: noseTensor,     // Shape: [1, 3, 56, 56]
        x_geom: geomTensor      // Shape: [1, 26, 2]
    };

    const outputs = await session.run(feeds);
    const rawEmbedding = outputs.face_embedding.data as Float32Array;
    
    // L2 Normalize
    let sum = 0;
    for (let i = 0; i < rawEmbedding.length; i++) sum += rawEmbedding[i] * rawEmbedding[i];
    const norm = Math.sqrt(sum);
    return rawEmbedding.map(x => x / norm);
}
```

#### C. Dart / Flutter (Mobile App Offline)
```dart
import 'package:onnxruntime/onnxruntime.dart';

class FlutterFaceInference {
  late OrtSession _session;

  Future<void> initModel() async {
    OrtEnv.instance.initialize();
    final options = OrtSessionOptions();
    _session = OrtSession.fromAsset("assets/custom_face_cnn.onnx", options);
  }

  Future<Float32List> getEmbedding(
    Float32List globalData,
    Float32List eyeData,
    Float32List noseData,
    Float32List geomData,
  ) async {
    final tGlobal = OrtValue.tensorFromList(globalData, [1, 3, 112, 112]);
    final tEye = OrtValue.tensorFromList(eyeData, [1, 3, 56, 112]);
    final tNose = OrtValue.tensorFromList(noseData, [1, 3, 56, 56]);
    final tGeom = OrtValue.tensorFromList(geomData, [1, 26, 2]);
    ...
    final rawEmb = outputs?[0]?.value as List<List<double>>;
    final embedding = Float32List.fromList(rawEmb[0].cast<double>());

    // Giải phóng bộ nhớ
    tGlobal.release(); tEye.release(); tNose.release(); tGeom.release();
    return l2Normalize(embedding);
  }

  Float32List l2Normalize(Float32List v) {
    double sum = v.map((x) => x * x).reduce((a, b) => a + b);
...
## 📌 Cập nhật 5: Kỹ Thuật Tăng Cường Dữ Liệu (Data Augmentation) Chống Quá Khớp Cục Bộ

Khi sử dụng dữ liệu từ thư mục `dataraw` để chuyển đổi sang `data_processed`, mô hình rất dễ gặp hiện tượng quá khớp (overfitting) do số lượng ảnh gốc của mỗi cá nhân thường hạn chế. Chúng tôi áp dụng quy trình tăng cường dữ liệu kép:

### 1. Tăng cường dữ liệu ngoại tuyến (Offline Augmentation)
Trong bước tiền xử lý ảnh từ `dataraw` sang `data_processed`, script tiền xử lý tự động tạo thêm các phiên bản ảnh biến đổi ánh sáng để đối phó với sự thay đổi độ tương phản thực tế:
- **Ảnh gốc**: Cắt và align mặt chuẩn.
- **Ảnh tăng độ sáng (`_bright`)**: Áp dụng tăng giá trị pixel hoặc gamma để giả lập môi trường ngược sáng hoặc đèn flash mạnh.
- **Ảnh giảm độ sáng (`_dark`)**: Giảm độ sáng và tăng độ nhiễu hạt nhẹ để giả lập môi trường thiếu sáng ban đêm.

### 2. Tăng cường dữ liệu trực tuyến (Online Augmentation)
Trong quá trình nạp dữ liệu bằng PyTorch `DataLoader`, chúng tôi áp dụng các lớp biến đổi ngẫu nhiên động trên từng batch:
* **Lật ngang ngẫu nhiên (Random Horizontal Flip)**: Xác suất $50\%$ để đối phó với tính đối xứng.
* **Xoay ngẫu nhiên (Random Rotation)**: Góc xoay nhỏ từ $-10^\circ$ đến $10^\circ$ giúp mô hình bất biến với góc nghiêng đầu nhẹ.
* **Thay đổi màu sắc (Color Jitter)**: Thay đổi nhỏ về độ tương phản, độ bão hòa màu và sắc thái để thích ứng với nhiều loại cảm biến camera khác nhau.
* **Giả lập che khuất động (Occlusion Synthesis)**: Tự động chèn ngẫu nhiên các đa giác màu xám hoặc các mảng tối đè lên vùng miệng/mắt với xác suất $20\%$ để mô hình học cách bỏ qua các vùng bị che khuất và dồn sự chú ý vào các vùng thông tin khả dụng khác.

---

## 📌 Cập nhật 6: Tối Ưu Hóa Tổng Quát Hóa: Dropout, Regularization, Batch Normalization & Dừng Sớm

Mô hình Custom Face CNN kết hợp cả dữ liệu hình học phẳng và đặc trưng tích chập sâu. Để tránh mô hình học vẹt (memorization), chúng tôi triển khai một hệ thống chuẩn hóa đa lớp nghiêm ngặt:

### 1. Cơ chế kiểm soát quá khớp trong kiến trúc
* **Spatial Dropout**: Áp dụng trong các block convolution của `PeriocularNet` và `NoseNet` với tỉ lệ tăng dần từ $0.1$ đến $0.2$. Khác với dropout thông thường, Spatial Dropout triệt tiêu toàn bộ một kênh đặc trưng, ép các kênh khác phải tự học các cấu trúc độc lập.
* **Fully Connected Dropout**: Đặt hệ số dropout cao ($0.3$ - $0.4$) ngay sau các lớp Fully Connected trước khi đưa vào module Attention chéo và phân lớp classification.
* **Batch Normalization**: Tích hợp sau mỗi lớp Conv và Linear để đảm bảo phân phối đặc trưng luôn ổn định trong khoảng trung bình bằng 0 và phương sai bằng 1, giúp tăng tốc độ hội tụ và giảm độ nhạy với các biến đổi phân phối dữ liệu đầu vào.
* **L1 & L2 Regularization**: Bổ sung hình phạt chuẩn hóa L1 ($\lambda = 10^{-5}$) trực tiếp vào hàm mất mát trong quá trình huấn luyện:
  $$Loss_{total} = Loss_{classification} + \lambda \sum |w_i|$$
  Đồng thời sử dụng bộ tối ưu hóa **AdamW** với trọng số suy giảm (weight decay = $10^{-4}$) để áp dụng chuẩn hóa L2 cho các trọng số mạng.

### 2. Cơ chế dừng sớm tự động lưu trữ ONNX (Graceful Interruption & Export)
Hệ thống giám sát **Validation Loss** trên tập dữ liệu kiểm thử.
* Nếu Validation Loss không cải thiện trong `patience` epoch liên tiếp, mô hình sẽ tự động dừng huấn luyện để tránh overfitting.
* **Bảo vệ ngắt quãng (Ctrl+C Handler)**: Nếu nhà phát triển gửi tín hiệu ngắt (`SIGINT` hoặc `SIGTERM`) bằng phím tắt Ctrl+C hoặc từ giao diện Flask, một trình bắt tín hiệu tùy chỉnh trong `train.py` sẽ chặn tiến trình thoát đột ngột. Nó sẽ tự động tải checkpoint tốt nhất trước đó (`checkpoint_best.pth`) và tiến hành xuất mô hình sang file ONNX `custom_face_cnn.onnx` rồi mới giải phóng bộ nhớ và dừng tiến trình một cách an toàn.

---

## 📌 Cập nhật 7: Trích Xuất Đặc Trưng Vùng Landmark & Tương Quan Cục Bộ

Thay vì chỉ phân tích tọa độ hình học thuần túy của các landmarks (dễ bị nhiễu do sai số định vị), hệ thống tích hợp thêm nhánh trích xuất đặc trưng vùng xung quanh từng điểm mốc hình học (Local Appearance Patches).

### 1. Phép toán trích xuất đặc trưng vùng (Local Patch Extraction)
Với mỗi điểm landmark $P_i = (x_i, y_i)$ trong số 26 điểm đặc trưng trên ảnh gốc $112 \times 112$:
1. Định vị một grid lấy mẫu (sampling grid) kích thước $15 \times 15$ pixels có tâm đặt chính xác tại tọa độ landmark $P_i$.
2. Áp dụng toán tử **`F.grid_sample`** của PyTorch để thực hiện nội suy song tuyến (bilinear interpolation) trích xuất vùng patch ảnh $3 \times 15 \times 15$ từ ảnh toàn mặt đầu vào $x_{global}$.
3. Đưa $26$ patches ảnh này qua một mạng CNN nhỏ chia sẻ trọng số `patch_conv` gồm:
   - `Conv2d(3, 16, kernel_size=3, padding=1)` $\rightarrow$ Kích thước: $16 \times 15 \times 15$
   - `BatchNorm2d` & `PReLU`
   - `Conv2d(16, 32, kernel_size=3, stride=2, padding=1)` $\rightarrow$ Kích thước: $32 \times 8 \times 8$
   - `AdaptiveAvgPool2d((1, 1))` $\rightarrow$ Phẳng hóa thành vector đặc trưng kết cấu cục bộ 32 chiều ($f_i \in \mathbb{R}^{32}$).

### 2. Tính toán ma trận tự tương quan cục bộ (Regional Correlation Matrix)
Để mô hình học được mối tương quan bề mặt giữa các vùng đặc trưng khác nhau (ví dụ: sự tương quan kết cấu da giữa đuôi mắt và thái dương, hoặc giữa cánh mũi và khóe miệng), chúng tôi tính toán ma trận tương quan Cosine Similarity kích thước $26 \times 26$:
1. Chuẩn hóa L2 các vector đặc trưng cục bộ:
   $$\hat{f}_i = \frac{f_i}{\|f_i\|_2}$$
2. Thực hiện phép nhân ma trận hàng loạt (Batched Matrix Multiplication) để tính tích vô hướng chéo:
   $$C = \hat{F} \hat{F}^T \in \mathbb{R}^{26 \times 26}$$
   Trong đó, $C_{i, j}$ chính là độ tương đồng Cosine giữa đặc trưng vùng của landmark $i$ và landmark $j$. Ma trận này phản ánh cấu trúc tương quan kết cấu da cục bộ cực kỳ ổn định và độc bản của mỗi người.

### 3. Tích hợp Self-Attention hình học nâng cao
Với mỗi landmark $i$, ta ghép nối (concatenate) 3 thành phần thông tin:
- Vector đặc trưng kết cấu vùng $f_i$ (32 chiều)
- Vector đặc trưng vị trí không gian chuẩn hóa $disp_i$ sau khi đi qua Position Embedding (32 chiều)
- Vector tương quan kết cấu $C_i$ của điểm đó với 25 điểm còn lại (26 chiều)

$$\text{Combined}_i = [f_i \ \Vert \ \text{PosEmb}(disp_i) \ \Vert \ C_i] \in \mathbb{R}^{90}$$

Toàn bộ 26 token Combined này được đưa qua một lớp Multi-Head Self-Attention để tìm các đặc trưng hình học không gian tự thích ứng cuối cùng, trước khi dẹt hóa và chiếu về vector hình học tổng hợp `f_geom` 64 chiều.

---

## 📌 Cập nhật 8: Hợp Nhất Đa Luồng Toàn Diện qua Attention Chéo (Cross-Stream Correlation Attention)

Để có một vector embedding đại diện hoàn hảo nhất cho khuôn mặt, hệ thống cần hợp nhất cả 4 luồng thông tin: luồng toàn mặt (`f_global`), luồng mắt (`f_eye`), luồng mũi (`f_nose`), và luồng hình học (`f_geom`). Thay vì sử dụng phép cộng hay ghép nối thô sơ, chúng tôi thiết kế module **CrossStreamCorrelationAttention** hoạt động dựa trên cơ chế tự chú ý chéo (Cross-Attention):

```text
[f_global (512-D)] ──► Lớp Chiếu ──► Token Global (128-D) ─┐
[f_eye    (128-D)] ──► Lớp Chiếu ──► Token Eye    (128-D) ─┼─► Stack thành [4, 128]
[f_nose    (64-D)] ──► Lớp Chiếu ──► Token Nose   (128-D) ─┤         │
[f_geom    (64-D)] ──► Lớp Chiếu ──► Token Geom   (128-D) ─┘         ▼
                                                                CNN 1D (Tương quan cục bộ)
                                                                     │
                                                                     ▼
                                                                Multi-Head Self-Attention (4 Heads)
                                                                     │
                                                                     ▼
                                                                Flatten & FC ──► Embedding 512-D
```

### 1. Đồng bộ hóa không gian đặc trưng (Feature Projection)
Bốn vector đặc trưng có số chiều khác nhau được chiếu về cùng một không gian ẩn (Token dimension = 128) thông qua các lớp LayerNorm và Linear độc lập, thu được 4 tokens đại diện:
$$t_{global}, t_{eye}, t_{nose}, t_{geom} \in \mathbb{R}^{128}$$

### 2. Tương quan cục bộ bằng CNN 1D
Xếp 4 tokens thành một ma trận $T \in \mathbb{R}^{4 \times 128}$.
Chúng tôi áp dụng một mạng **CNN 1D** dọc theo chiều token (sequence length = 4) với kích thước kernel=3 để học các mối liên hệ và tương quan cục bộ trực tiếp giữa các luồng đặc trưng lân cận (ví dụ: tương quan trực tiếp giữa luồng Mắt và luồng Mũi, hoặc luồng Mũi và luồng Hình học).

### 3. Tự chú ý toàn cục đa đầu (Multi-Head Self-Attention)
Đưa các tokens đã qua CNN 1D vào lớp **Multi-Head Self-Attention (4 đầu)** để mô hình hóa mối quan hệ tương hỗ chéo toàn cục giữa tất cả các luồng đặc trưng.
* Cơ chế này cho phép các luồng tự động bổ trợ thông tin cho nhau (ví dụ: nếu luồng Mũi bị nhiễu do khẩu trang, đầu tự chú ý sẽ tăng cường lấy thông tin từ luồng Mắt và luồng Hình học để bù đắp).

### 4. Phẳng hóa và Chiếu đầu ra
Đầu ra sau khi qua Attention được cộng Residual, chuẩn hóa qua LayerNorm, phẳng hóa thành vector 512 chiều ($4 \times 128$) và đưa qua lớp Fully Connected cuối cùng để sinh ra vector embedding 512 chiều chuẩn hóa L2 hoàn hảo nhất.

### 5. Cơ chế tương thích ngược (Backward Compatibility)
Mặc dù sử dụng ma trận Attention chéo $4 \times 4$ nội bộ, mô hình cần giữ nguyên các cổng đầu ra của file ONNX cũ để tránh làm vỡ các ứng dụng client/webcam HUD đang chạy. 
Hệ thống giải quyết bằng cách lấy giá trị trung bình dọc theo chiều truy vấn của ma trận trọng số Attention để nén từ kích thước $(B, 4, 4)$ về vector trọng số chú ý 3 chiều $(B, 3)$ đại diện cho độ tin cậy của Mắt, Mũi và Toàn mặt, trả về chính xác qua cổng đầu ra `attention_weights`.

---

## 📌 Cập nhật 9: Tối Ưu Hóa Kích Thước Patch Vùng Landmark lên 15x15 (Đường chéo 20px)

Kích thước của vùng ảnh cắt xung quanh các landmarks (Patch size) ảnh hưởng trực tiếp đến khả năng trích xuất các đặc trưng kết cấu cục bộ. 

### 1. Phân tích toán học về kích thước patch
Trong cấu hình ban đầu, chúng tôi sử dụng patch kích thước $11 \times 11$ pixels.
* Một hình vuông kích thước $11 \times 11$ pixels trên ảnh $112 \times 112$ có đường chéo là:
  $$\text{Diagonal}_{11} = \sqrt{11^2 + 11^2} = \sqrt{242} \approx 15.55 \text{ pixels}$$
  Điều này chỉ tương đương đường chéo ~15px trên ảnh thực tế, giới hạn lượng ngữ cảnh (context) xung quanh các điểm mốc nhạy cảm như khóe mắt hay cánh mũi.

Để đạt được đường chéo **20px** theo yêu cầu kỹ thuật thực tế giúp nhận diện tốt hơn cấu trúc vi mô, kích thước cạnh hình vuông cần thỏa mãn:
$$\text{Cạnh} = \frac{\text{Đường chéo}}{\sqrt{2}} = \frac{20}{\sqrt{2}} \approx 14.14 \text{ pixels}$$
Vì kích thước cạnh của patch cần là số lẻ để có điểm tâm landmark đối xứng chính xác, chúng tôi chọn **cạnh hình vuông là 15 pixels**.
* Một hình vuông kích thước $15 \times 15$ pixels (bán kính offset 7px từ tâm) có đường chéo thực tế là:
  $$\text{Diagonal}_{15} = \sqrt{15^2 + 15^2} = \sqrt{450} \approx 21.21 \text{ pixels}$$
  Đáp ứng hoàn hảo yêu cầu đường chéo tối thiểu 20px.

### 2. Xác minh hiệu quả thực tế
Sau khi nâng cấp kích thước patch lên $15 \times 15$ và huấn luyện mô hình:
* **Độ tương đồng Cosine** khi so khớp offline bằng FAISS đối với đối tượng thử nghiệm `'55503'` đã **tăng từ 0.8929 lên 0.8991**.
* Sự cải thiện này chứng minh rằng việc mở rộng vùng quan sát xung quanh landmarks lên $15 \times 15$ pixels giúp nhánh `GeometricNet` thu nhận nhiều thông tin ngữ cảnh chất lượng hơn, tăng độ phân biệt đặc trưng và cải thiện khả năng chống nhiễu khi góc mặt thay đổi.
