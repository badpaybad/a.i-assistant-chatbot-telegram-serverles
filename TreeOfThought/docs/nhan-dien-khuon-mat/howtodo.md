...
### 8.5. Cấu hình động Tham số Huấn luyện (Dynamic Training Hyper-parameters)
- **Cấu hình trực quan trên UI**:
  - Tích hợp bảng cấu hình tham số (`div.training-params-section`) ngay phía trên nút bấm *"Đào tạo"*.
  - Giao diện cung cấp các trường nhập liệu thân thiện: `Epochs` (1-500), `Batch Size` (4-64), `Learning Rate`, `Align Mode` (Advanced/Standard), và `Device` (CPU/CUDA).
...
- **Truyền động Backend qua Query String (SSE)**:
  - Hàm `streamTraining` trong `NhanDienKhuonMatService` nhận các tham số từ UI và nối chúng động vào URL SSE: `&epochs={epochs}&batchSize={batchSize}&learningRate={learningRate}&alignMode={alignMode}&device={device}`.
  - API `TrainStream` tại `FaceDetectionController.cs` tiếp nhận các tham số này thông qua `[FromQuery]`, sau đó truyền động trực tiếp vào các cờ lệnh (`--epochs`, `--batch_size`, `--learning_rate`, `--align_mode`, `--device`) khi khởi tạo và khởi chạy tiến trình Python `main.py`.
  - Hỗ trợ định dạng dấu thập phân chuẩn hệ thống qua `System.Globalization.CultureInfo.InvariantCulture` tránh lỗi chuyển đổi chuỗi trên các môi trường đa ngôn ngữ.
