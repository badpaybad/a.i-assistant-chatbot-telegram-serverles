...
### 8.5. Cấu hình động Tham số Huấn luyện & Tham số Biên độ góc (Dynamic Training & Margin Hyper-parameters)
- **Cấu hình trực quan trên UI (Frontend Angular)**:
  - Tích hợp bảng cấu hình tham số (`div.training-params-section`) ngay phía trên nút bấm *"Đào tạo"*.
  - Giao diện cung cấp các trường nhập liệu thân thiện: `Epochs` (1-500), `Batch Size` (4-64), `Learning Rate`, `Align Mode` (Advanced/Standard), `Device` (CPU/CUDA), và **`Margin` (Biên độ góc)** (0.1 - 1.0, mặc định `0.50`).
  - Giao diện sử dụng trường nhập liệu `<nz-input-number [nzMin]="0.1" [nzMax]="1.0" [nzStep]="0.05" [(ngModel)]="margin" [nzDisabled]="isTraining">` cho Margin.
  - Tự động vô hiệu hóa (disabled) tất cả các trường cấu hình khi đang trong quá trình train để đảm bảo tính nhất quán của tham số huấn luyện.
- **Truyền động Backend qua Query String (SSE) & Command Model (C#)**:
  - Hàm `streamTraining` trong `NhanDienKhuonMatService` nhận tham số `margin` từ UI và nối động vào URL SSE: `&epochs={epochs}&batchSize={batchSize}&learningRate={learningRate}&alignMode={alignMode}&device={device}&margin={margin}`.
  - API `TrainStream` tại `FaceDetectionController.cs` tiếp nhận tham số `[FromQuery] double margin = 0.50`, ánh xạ nó vào thuộc tính `Margin` của lớp `TrainArcFaceModelCommand`.
  - `FaceDetectionCommandHandler.cs` tiếp nhận `TrainArcFaceModelCommand`, sau đó truyền động tham số này vào cờ lệnh `--margin` khi khởi tạo và khởi chạy tiến trình Python `main.py`:
    ```csharp
    $"--margin {command.Margin.ToString(System.Globalization.CultureInfo.InvariantCulture)}"
    ```
  - Định dạng dấu thập phân dấu chấm chuẩn được bảo toàn qua `System.Globalization.CultureInfo.InvariantCulture` giúp tránh lỗi chuyển đổi chuỗi trên các môi trường đa ngôn ngữ.
- **Cơ chế Dynamic Margin Warmup trong Python (`main.py`)**:
  - Để giải quyết lỗi toán học làm sập Accuracy về đúng 0.00% và gây bão hòa logit (Loss ~33) ở các epoch đầu do các vector phân bố hỗn loạn ban đầu, pipeline tự động kích hoạt **Margin Warmup**:
    - *Giai đoạn 1 (Epoch 1-5)*: Tạm thời đưa margin về **`0.0`** (Cosine Similarity thuần túy) để mô hình gom cụm thô các user mới một cách dễ dàng mà không bị trừng phạt góc, giúp Acc nhanh chóng đạt mức cao (>80%).
    - *Giai đoạn 2 (Epoch 6 trở đi)*: Khôi phục margin đích (`--margin` nhận từ C#, mặc định `0.50`) để tiếp tục siết chặt ranh giới quyết định sâu sắc mà không làm sập Accuracy.

### 8.6. Tích hợp Xử lý Phát hiện & Trích xuất Landmarks trên Server C# (Cập nhật 2026-05-30 14:35:35)

Nhằm đáp ứng yêu cầu chạy ngầm và xử lý đối sánh khuôn mặt offline trực tiếp trên backend C# mà không phụ thuộc vào Client gửi eye coordinates, hệ thống đã tích hợp toàn diện 2 mô hình ONNX cục bộ thay thế cho Google MediaPipe Task và TFLite:

1.  **Mô hình phát hiện khuôn mặt (`face_detection.onnx`):** Chuyển đổi tương ứng từ `blaze_face_short_range.tflite` của Google. Chạy trên tensor đầu vào dạng `[1, 3, 272, 160]` (RGB chuẩn hóa về khoảng `[-1, 1]`).
2.  **Mô hình điểm mốc khuôn mặt (`face_landmark.onnx`):** Chuyển đổi tương ứng từ mô hình Face Mesh của `face_landmarker.task`. Chạy trên tensor đầu vào dạng `[1, 3, 128, 128]` (RGB chuẩn hóa về khoảng `[0, 1]`), trả về tọa độ 148 điểm landmarks 3D khuôn mặt (444 floats), Euler angles và độ tin cậy.

#### Sơ đồ cấu trúc lớp C# xử lý ONNX Pipeline:

```mermaid
graph TD
    A[FaceProcessController.cs] -->|Nạp models từ arcfacemodels/| B[InferenceSession Thread-Safe]
    A -->|POST /api/face-process/detect| C[Detect Face: Chạy BlazeFace ONNX 160x272]
    A -->|POST /api/face-process/landmarks| D[Extract Landmarks: Chạy FaceMesh ONNX 128x128]
    A -->|POST /api/face-process/align| E[Bilinear Affine Align: Tạo ảnh 112x112 chuẩn]
```

*   **Tệp tin mới được tích hợp:**
    *   [FaceProcessController.cs](file:///work/a.i-assistant-chatbot-telegram-serverles/TreeOfThought/backend/nhan-dien-khuon-mat/Controllers/FaceProcessController.cs)
*   **Endpoints hoạt động:**
    *   `POST /api/face-process/detect`: Chạy suy luận phát hiện khuôn mặt BlazeFace, trả về hình dạng tensor đầu ra.
    *   `POST /api/face-process/landmarks`: Trích xuất 148 điểm mốc 3D (x, y, z), trả về vị trí hốc mắt trái (landmark `33`) và mắt phải (landmark `133`) làm tham chiếu.
    *   `POST /api/face-process/align`: Thực hiện phép biến đổi tương đồng Affine xoay thẳng khuôn mặt và nội suy song tuyến (Bilinear Interpolation) hoàn toàn trên bộ nhớ RAM để xuất ra ảnh chân dung đạt chuẩn ArcFace `112x112`.
*   **Tối ưu hóa tài nguyên:** 
    Sử dụng lớp cache `ConcurrentDictionary<string, Lazy<InferenceSession>>` giúp tải và giữ mô hình ONNX trong bộ nhớ RAM tĩnh của máy chủ một cách thread-safe, loại bỏ chi phí đọc đĩa và khởi tạo lại session ở mỗi request.
