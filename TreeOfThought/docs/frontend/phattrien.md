...
- **`projects/tot/core`**: Thư viện cốt lõi (Core Library).
  - Chứa: `HttpClientService`, `AuthService`, `Interceptors`, `Guards`, `Claims`, `MessageBusService`, `ComponentRegistryService`, `I18n Utilities (Transloco)`.
...
### 8.1. Centralized Core I18n
To đảm bảo tính nhất quán và đóng gói (encapsulation), toàn bộ logic cấu hình đa ngôn ngữ được đặt tại **`@tot/core`**:
- **Loader**: `TranslocoHttpLoader` được định nghĩa trong core để thống nhất cách nạp file JSON.
- **Provider**: Sử dụng hàm `provideTotI18n()` từ `@tot/core` để khởi tạo hệ thống trong `app.config.ts`.
  ```typescript
  provideTotI18n({
...
- **Standalone Components**: Bắt buộc import `TranslocoModule` từ `@jsverse/transloco` vào mảng `imports` của component.

### 8.4. Cấu hình Runtime
