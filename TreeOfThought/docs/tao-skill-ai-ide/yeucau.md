...
Ý định:
- Cần xuất bộ quy tắc đã được định nghĩa trong `TreeOfThought/howtodev/yeucau.md` ra thành các file cấu hình (rule/prompt) chuẩn.
...
- Mục tiêu là để bắt kỳ AI nào cũng tuân thủ đúng quy trình làm việc `tot-dev`:
  + Tuân thủ KISS.
  + Đọc `yeucau.md` -> Viết `phattrien.md` -> Đợi xác nhận -> Mới code.
