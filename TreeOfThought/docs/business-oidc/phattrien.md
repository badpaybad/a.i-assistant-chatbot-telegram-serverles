...
- [ ] **Tác vụ 1: Chuyển đổi sang Server-side Pagination:**
  - Cập nhật FE gọi các API `/users`, `/roles`, `/claims` truyền kèm `pageIndex` và `pageSize`.
  - Thay đổi cấu trúc binding từ flat array sang `{ items, total }`, tắt cờ `frontPagination` ở `tot-table`.
