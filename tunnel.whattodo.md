...
- dùng subdomain public được cấp bởi cloudflare tunnel và nginx để làm proxy cho
    ...
    - khi có subdomain từ tunnel cần gửi thông tin url subdomain, port rust desk, path uri proxy pass cho local port nào , gửi thông qua telegram tới user @badpaybad 
...
- nếu cần pull code hoặc cài đặt rustdesk server, cloudflared , nginx, các service cần thiết để tự động cài đặt khi cần 
